...
if(isset($_POST['updatefees'])){
  ...
  $amt=intval($_POST['fees']);

  $qcheck="select * from fees where classId=$class";

  $resq=mysqli_query($con,$qcheck);

  if(mysqli_num_rows($resq) < 1){

      $qry="INSERT INTO FEES(classId,amount) VALUES ('$class','$amt')";
      ...
      $qry="update FEES set  amount=$amt where classId=$class";
  ...
  <body>
    ...
    <main class="main">
      ...
      <section class="alladm">
        <h3>ADD / UPDATE FEES STRUCTURE</h3>
      </section>
  ...
  <script>
          ...
          function closeAlert() {
            ...
            if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="updated") {
                echo "showAlert('✅ Fees Updated Successfully!');";
                unset($_SESSION['update_success']);
            }

            else if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="notupdated") {
                echo "showAlert('⚠️ Error, Fees Not Updated!');";
                unset($_SESSION['update_success']);
            }


            else if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="added") {
                echo "showAlert('✅ Fees Added Successfully!');";
                unset($_SESSION['update_success']);
            }

            else if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="notadded") {
                echo "showAlert('⚠️ Error, Fees Not Added!');";
                unset($_SESSION['update_success']);
            }
