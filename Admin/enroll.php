...
if(isset($_SESSION['admin'])){
    ...
    if (mysqli_num_rows($r) > 0) {
            ...
            $output .= "
                <tr>
                    ...
                    <td>$gen</td>
                     <td>$cl</td>
                    <td>$prev</td>
                     <td><button onclick='downloadFile($sid)' id='app'><i class='fas fa-file-download'></i></button></td>
                    <td><a href='approve.php?viewid=$sid'><button id='app'>Accept<i class='far fa-check-circle'></i></button></a>
                    <a href='reject.php?removeid=$sid'><button id='rej'>Reject<i class='fa-regular fa-rectangle-xmark'></i></button></a></td>
                </tr>
            ";
        }
    }
        ...
        $output.="
                    <tr>
                        <td colspan='10'><h4> No New Enrollment </h4></td>
                    </tr>       
        ";
    }
...
<html lang="en">
  ...
  <script src="jsfile.js"></script>
    ...
    <script>
        function downloadFile(fileId) {
            window.location.href = 'downloadmrks.php?file_id=' + fileId;
        }
    </script>
  ...
  <script>
          ...
          function closeAlert() {
            ...
            if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="reject") {
                echo "showAlert('⚠️ Applicant Successfully Rejected....!');";
                unset($_SESSION['update_success']);
            }

            else if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="approve") {
                echo "showAlert('✅ Enrollment Approved Successfully!');";
                unset($_SESSION['update_success']);
            }

            else if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="error") {
                echo "showAlert('⚠️ Error In Performing Action....!');";
                unset($_SESSION['update_success']);
            }


            else if (isset($_SESSION['update_success']) && $_SESSION['update_success']=="roll") {
                echo "showAlert('⚠️ Oops, Given Roll Number Already Exist In The Class, Please Enter Unique Roll Number!');";
                unset($_SESSION['update_success']);
            }
